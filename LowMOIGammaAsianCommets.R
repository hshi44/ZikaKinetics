...
##Defineing the function ssq_msas to integrate and evaluate the prediction error of parameter set p##
ssq_msas <- function(p){
...
#for (j in 1:2){
  #for (l in 1:2){
    ...
    cp <- c(as.numeric(!(pi>(spc$aspa+spc$aspd))),as.numeric(!(pp<(spc$aspa-spc$aspd))))
