...
#for(j in 1:10){
#  for(l in 1:10){
    parms<-c(taue=19.8, taui=18,k5=29)
    fit_af_gd_os <- nls.lm(par=parms,fn=ssq_gd, lower = c (1e-4,1e-4,1e-4),control = nls.lm.control(maxiter = 500))#Nonlinear Least Squares to minimize the prediction errors#
    ##Integration and calculation of the prediction error##
    out <- ode(y0,virus$time,gamma_test,fit_af_gd_os$par)
    prd <- data.frame(out)
    res <- prd$V / virus$V-1
    put <- rbind(put,c(parms,fit_af_gd_os$par,nv,res%*%res))
#  }
#}
...
points(virus)
write.csv(put,file = 'OSGD_African_modeloutput.csv')

##The parameters and the evaluation as the output##
write.csv(put,file = paste0(DVdir,'OSGD_African_parameters.csv'))
